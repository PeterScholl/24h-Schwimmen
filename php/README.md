...
│
├── flask_templates/       ← JS-Templates (Python UND PHP nutzen diese)
│   ├── main_v2.js         ← Erfassungsseite v2 (Legacy)
│   ├── main_v3.js         ← Erfassungsseite v3 (aktueller Standard)
│   ├── view.js            ← Ergebnisseite
│   └── view2.js           ← Ergebnisseite Slideshow
│
...
> **Hinweis zu `flask_templates/`:** Die JS-Dateien (`main_v3.js`, `view.js` etc.) werden vom PHP-Backend direkt aus diesem Verzeichnis gelesen. Die Jinja2-`{{ variable }}`-Syntax wird dabei via PHP `str_replace` ersetzt. Die HTML-Templates werden **nicht** von PHP genutzt – dafür gibt es die eigenen PHP-Templates in `php/templates/`.
...
| **Sessions** | Flask signierte Cookies (24 h) | PHP native Server-Sessions (24 h) |
...
| `/login` | Login-Seite |
| `/logout` | Abmelden |
| `/v2` | Erfassungsseite v2 (Legacy) |
| `/v3` | Erfassungsseite v3 (aktueller Standard) |
...
| `/view` | Anzeigebildschirm |
| `/view2` | Anzeigebildschirm Slideshow |
| `/main_v3.js`, `/view.js`, … | Dynamisch gerenderte JS-Dateien |
...
Die `config.json` im Projektwurzelverzeichnis anpassen:
...
Alle Werte außer `flask_secret_key` können auch über den Admin-Bereich → **Konfiguration** angepasst werden, ohne die Datei manuell zu bearbeiten. Änderungen gelten beim PHP-Backend sofort beim nächsten Request, ohne Neustart.
...
Schreibzugriff wird benötigt auf:
- `/var/www/24hschwimmen/config.json` (Admin-Bereich → Konfiguration)
- `/var/www/24hschwimmen/data/` (Logdatei)
...
Alle Werte lassen sich auch über den Admin-Bereich → **Konfiguration** bearbeiten. Änderungen werden in `config.json` gespeichert und gelten beim nächsten Request sofort (kein Neustart nötig).
...
| `startzeit` | ISO 8601 | Wettkampfbeginn UTC (für Zeitberechnungen in der View) |
| `session_lifetime_h` | Integer | Dauer einer Anmelde-Session in Stunden (Standard: 24) |
| `fade_time_s` | Integer | Sekunden Inaktivität bis Schwimmerkarte ausgegraut und auf Bahn 0 verschoben wird. `0` = deaktiviert |
| `max_bahnen` | Integer | Anzahl Bahnbuttons auf der Erfassungsseite v3 |
| `v3_timer_dauer_ms` | Integer | Wartezeit in ms nach Kachelklick bis zur automatischen Übertragung (Standard: 5000) |
| `v3_senden_btn` | Integer | `1` = Senden-Button oben rechts einblenden, `0` = ausgeblendet |
| `mobile_cards_col` | Integer | Spaltenanzahl auf Mobilgeräten in v2 |
...
| `flask_secret_key` | String | **Python only** – Schlüssel für Flask-Sessions; nicht über Admin-UI editierbar |
...
| `ERROR` | Fehler | DB-Fehler, ungültige Parameter, Rollback |
...
**JS-Dateien (main_v3.js etc.) geben 500 zurück**
...
→ `config.json` muss für `www-data` lesbar und schreibbar sein. Prüfen mit `ls -la config.json`.
